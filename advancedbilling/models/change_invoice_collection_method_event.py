    ...
    def validate(cls, dictionary):
        ...
        if isinstance(dictionary, cls):
            return APIHelper.is_valid_type(value=dictionary.id, type_callable=lambda value: isinstance(value, int)) \
                and APIHelper.is_valid_type(value=dictionary.timestamp, type_callable=lambda value: isinstance(value, APIHelper.RFC3339DateTime)) \
                and APIHelper.is_valid_type(value=dictionary.invoice, type_callable=lambda value: Invoice.validate(value)) \
                and APIHelper.is_valid_type(value=dictionary.event_type, type_callable=lambda value: InvoiceEventType.validate(value)) \
                and APIHelper.is_valid_type(value=dictionary.event_data, type_callable=lambda value: ChangeInvoiceCollectionMethodEventData.validate(value))

        if not isinstance(dictionary, dict):
            return False

        return APIHelper.is_valid_type(value=dictionary.get('id'), type_callable=lambda value: isinstance(value, int)) \
            and APIHelper.is_valid_type(value=dictionary.get('timestamp'), type_callable=lambda value: isinstance(value, str)) \
            and APIHelper.is_valid_type(value=dictionary.get('invoice'), type_callable=lambda value: Invoice.validate(value)) \
            and APIHelper.is_valid_type(value=dictionary.get('event_type'), type_callable=lambda value: InvoiceEventType.validate(value)) \
            and APIHelper.is_valid_type(value=dictionary.get('event_data'), type_callable=lambda value: ChangeInvoiceCollectionMethodEventData.validate(value))
